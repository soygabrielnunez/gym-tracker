...
onMounted(() => {
  if (workout) {
    ...
    // Deep copy and ensure backward compatibility for restTime
    exercises.value = JSON.parse(JSON.stringify(workout.exercises)).map((ex: any) => {
      if (!ex.restTime) {
        ex.restTime = { value: null, unit: 'seconds' };
      }
      return ex;
    });
  } else {
